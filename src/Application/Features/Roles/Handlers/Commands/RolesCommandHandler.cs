...
namespace Application.Features.Roles.Handlers.Commands;
    ...
    IRequestHandler<AssignClaimToUserCommand, BaseResponse<string>>
{
    ...
    public async Task<BaseResponse<string>> Handle(AddClaimToRoleCommand request,
        CancellationToken cancellationToken)
    {
        return await _roleService.AddClaimToRole(request);
    }

    public async Task<BaseResponse<string>> Handle(AssignClaimToUserCommand request,
        CancellationToken cancellationToken)
    {
        return await _roleService.AddClaimToUser(request);
    }

    public async Task<BaseResponse<string>> Handle(AssignRoleToUserCommand request,
        CancellationToken cancellationToken)
    {
